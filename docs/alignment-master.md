...
| 4 | 视频信息 / 缩略图 / 时长 / 频道 / 格式表展示 | `ytsage/gui/ytsage_gui_main.py`, `ytsage/gui/ytsage_gui_video_info.py`, `ytsage/gui/ytsage_gui_format_table.py` | `src/pages/DownloadPage.vue` | `src-tauri/src/models/mod.rs`, `src-tauri/src/services/analysis_service.rs` | `部分可用` | 60% | `P0` | 下载页已可展示缩略图、标题、频道、时长、播放列表标记与格式表，格式表已补到编码/FPS/HDR；但仍缺原版的格式选择联动、高低质量分级着色、音视频模式切换与字幕/赞助片段联动入口 | 2026-05-05 |
...
| 6 | 真实下载执行链 | `ytsage/core/ytsage_downloader.py` | `src/pages/BatchPage.vue`, `src/stores/queue.ts`, `src/stores/download.ts` | `src-tauri/src/services/download_service.rs`, `src-tauri/src/services/queue_service.rs` | `部分可用` | 40% | `P0` | 已接入 `yt-dlp --newline` 参数组装、真实子进程 PID 登记与流式进度/速度/ETA 回写，取消态也不会被完成收尾覆盖；但仍缺最终输出路径跟踪、FFmpeg 后处理结果收口和更完整高级参数映射验证 | 2026-05-05 |
| 7 | 队列状态机与任务控制 | `ytsage/gui/ytsage_gui_main.py`, `ytsage/core/ytsage_downloader.py` | `src/pages/BatchPage.vue`, `src/stores/queue.ts` | `src-tauri/src/commands/mod.rs`, `src-tauri/src/services/queue_service.rs` | `部分可用` | 55% | `P0` | 暂停、恢复、取消已接入真实子进程控制，重试会重新调度下载，前端继续通过 `queue_state_changed` 同步细粒度进度；但仍缺多任务串行调度、批次推进和真实下载产物落库后的状态收口 | 2026-05-05 |
